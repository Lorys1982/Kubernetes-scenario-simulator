...
var defaultKubeconfigPath = path.Join(os.Getenv("HOME"), ".kube", "config")
var DefaultKubeconfig *KubeContexts
var none = None[global.LogCommandInfo]()

// KubeContexts struct is to keep track of all the contexts
...
type KubeContexts struct {
	...
	ClusterNames []KubeCluster `yaml:"clusters"`
}
...
// UpdateDefaultKubeconfig function is needed if the user creates/deletes clusters
// during a scenario, to mirror the kubeconfig saved in the program with the updated file
func UpdateDefaultKubeconfig() {
	yamlFile, err := os.ReadFile(defaultKubeconfigPath)
	if err != nil {
		writers.CrashLog(err.Error(), none)
	}
	err = yaml.Unmarshal(yamlFile, &DefaultKubeconfig)
	if err != nil {
		writers.CrashLog(err.Error(), none)
	}
}
...
func GetClusterNameKubeconf() []KubeCluster {
	return DefaultKubeconfig.ClusterNames
}
	...
	for i := range commandsConf {
		for j := range commandsConf[i].Spec.Queues {
			if commandsConf[i].Spec.Queues[j].Kubeconfig == "" {
				commandsConf[i].Spec.Queues[j].Kubeconfig = defaultKubeconfigPath
			}
...
func GenContexts(kubeconf string) KubeContexts {
	if defaultKubeconfigPath == kubeconf {
