...
var DefaultKubeContext *KubeContexts
...
func UpdateDefaultKubeconfig() {
	...
	err = yaml.Unmarshal(yamlFile, &DefaultKubeContext)
	if err != nil {
		writers.CrashLog(err.Error())
	}
}
	...
	for i := range commandsConf {
		for j := range commandsConf[i].Spec.Queues {
			...
			if commandsConf[i].Spec.Queues[j].KubeContext.Name == "" {
				writers.CrashLog("Custom Kubeconfig file does not contain a context with the correct cluster")
			}
		}
	}
}
